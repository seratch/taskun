...
public class CrontabParser {
	...
	static Logger log = Logger
			.getLogger(CrontabParser.class.getCanonicalName());
	...
	public Crontab parseLine(RawCrontabLine line) {
		...
		try {
			...
			log.severe("Invalid crontab expression...");
...
//								boolean isAfterFixedTimestamp = (CalendarUtil.get24Hour(currentTime) * 60
//										+ CalendarUtil.getMinute(currentTime)) > (hour * 60 + minute);
								boolean isNextInvocationNotInThisDate =
										currentTime.getTimeInMillis() > nextInvocationTime.getTimeInMillis();
								if (isDayOfWeekNotContainedInCrontab
//										|| isAfterFixedTimestamp
					...
					if (isHourModified || crontab.hourElement.isFixedValue()) {
						if (isDayModified || crontab.dayElement.isNotWildcard()) {
							if (isDayModified || crontab.dayElement.isFixedValue()) {
								if (isMonthModified
										|| crontab.monthElement.isNotWildcard()) {
									if (isMonthModified
											|| crontab.monthElement.isFixedValue()) {
										// cannot specify year at crontab
										nextInvocationTime
												.add(Calendar.YEAR, 1);
									} else {
	...
	public int getFixedNextInvocationValue(int currentValue, int maxValue,
										   CrontabElement element) {
	...
	public Crontab getIntervalInvocationCrontab(Crontab crontab,
												String[] elements) {
